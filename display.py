...
def printDataToFile( data, outfile, x ):    
    ...
    fileWrite.write("\n\nStar number: %d \n" % x)
    fileWrite.write(tabulate(data, headers = ["Observation ID", "Observation Name", "Creator", "Project ID", "Right Ascension", "Declination", "Start Time (UTC)", "Frequency (MHz)", "Correlation Mode", "Duration (sec)"]))
